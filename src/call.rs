...
pub fn single_cell_bulk(matches: &clap::ArgMatches) -> Result<(), Box<Error>> {
    ...
    // test that the `Event`s exactly cover the `Event` space
    {
        ...
        for event in events.iter() {
            for daf in event.af_case.iter() {
                ...
                if event.af_control.left_exclusive {
                    start += 0.0000000001
                };
                event_collection
                    .get_mut(&daf)
                    .unwrap()
                    .insert(start, &event.af_control);
            }
        }
    ...
    // init joint model
    let mut joint_model =
        libprosic::model::PairCaller::new(single_sample, bulk_sample, prior_model);
