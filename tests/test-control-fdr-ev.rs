...
fn test_control_fdr_ado_0_04() {
    ...
    let _output = control_fdr_ev
        ...
        .arg("0.04")
        .arg("--events")
        .arg("ADO_TO_ALT,ADO_TO_REF")
        .arg("--var")
        .arg("SNV")
        .arg("--output")
        .arg("test-out_ADO_0-04.bcf")
        .output()
        .expect_success();
...
fn test_control_fdr_alt_filter_one() {
    ...
    let _output = control_fdr_ev
        ...
        .arg(
            control_fdr_ev
                .src_path("tests/control-fdr-tests/alt-prob-sum-above-one_test-sites.bcf"),
        ).arg("--fdr")
        .arg("0.000000001")
        .arg("--events")
        .arg("ADO_TO_REF,ADO_TO_ALT,HOM_ALT,HET,ERR_REF")
        .arg("--var")
        .arg("SNV")
        .arg("--output")
        .arg("test-out_ALT_filter-one.bcf")
        .output()
        .expect_success();

    //test that the output BCF file is the same as before
    let expected =
        control_fdr_ev.src_path("tests/control-fdr-tests/expected-out_ALT_filter-one.bcf");
    let mut bcf_expected: Vec<u8> = Vec::new();
    if let Ok(_) = File::open(expected).unwrap().read_to_end(&mut bcf_expected) {
...
fn test_control_fdr_alt_filter_none() {
    ...
    let _output = control_fdr_ev
        ...
        .arg("SNV")
        .arg("--output")
        .arg("test-out_ALT_filter-none.bcf")
        .output()
        .expect_success();

    //test that the output BCF file is the same as before
    let expected =
        control_fdr_ev.src_path("tests/control-fdr-tests/expected-out_ALT_filter-none.bcf");
    let mut bcf_expected: Vec<u8> = Vec::new();
    if let Ok(_) = File::open(expected).unwrap().read_to_end(&mut bcf_expected) {
